...
func (b *Tree) findKeyAndPath(key string, pathBuf []slot) (found bool, path []slot) {
	path = pathBuf[:0] // we track the path to the key's slot
	var index int
	var node *Node = b.root // start search at the top
	for !node.isLeaf() {
...
func (node *Node) findSlot(key string) (bool, int) {
	items, itemcnt := node.items, len(node.items)
	slotinx := sort.Search(itemcnt, func(i int) bool {
		return items[i].key >= key // will find the smallest i for which this is true
	})
	return slotinx < itemcnt, slotinx
}
